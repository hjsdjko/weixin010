	...
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0px","position":"relative","background":"#ffffff","height":"auto"}'>
				...
				<view :style='{"margin":"12rpx 0 12rpx 0","borderColor":"#cccccc","flexWrap":"wrap","borderWidth":"0 0 0px 0","background":"#ffffff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="detail-list-item title">
					...
					<view :style='{"padding":"0px 0 0 0","margin":"0 0 0 0","lineHeight":"48rpx","fontSize":"28rpx","color":"#666666","flex":"1"}' class="text">{{detail.faburiqi}}</view>
				</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discussdongtaixinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdongtaixinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('dongtaixinxi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','dongtaixinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','dongtaixinxi');
			...
			async init(){
				...
				let res = await this.$api.info('dongtaixinxi', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdongtaixinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdongtaixinxi/add-or-update?refid=${this.id}`)
			},
			// 获取赞踩
			async getThumbsup() {
				if(!this.user){
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('dongtaixinxi', _this.detail);
